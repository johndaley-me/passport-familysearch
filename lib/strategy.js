 ...
 *  - `display`   Request a custom display option, i.e. page, popup, touch, wap, lite, etc.
 *  - `username`  Pre-populate the sign-in page with the provided username
 *  - `icid`      Analytics tracking id
 *  - `flow`      Sign-in page variances per flow
 *  - `prompt`    Specifies whether the Authorization server prompts the End-User for reauthentication 
 *
 * @param {Object} options
 * @return {Object}
 * @api protected
...
Strategy.prototype.authorizationParams = function (options) {
  ...
  if(this._signer) {
    params.client_secret = this._signer();
  }

  // pass approved parameters to the authorization request
  ['referrer', 'display', 'username', 'userName', 'icid', 'flow', 'prompt'].forEach(param => {
    if (options[param]) {
      params[param] = options[param];
    }
  });
  // legacy support for low_bandwidth parameter
  if (options.display === 'lite' || options.display === 'wap') {
